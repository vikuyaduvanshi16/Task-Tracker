// Learner Tracker — Service Worker v8 (auto-tick fix)
const SW_VERSION = 'ait-sw-v8';
...
self.addEventListener('notificationclick', e => {
  ...
  if (action === 'done') {
    e.waitUntil(
      self.clients.matchAll({type:'window', includeUncontrolled:true}).then(clients => {
        ...
        // Try to find an already-open window and navigate it to the markdone URL
        for (const client of clients) {
          if ('navigate' in client) {
            client.focus();
            return client.navigate(markUrl);
          }
        }
        // No open window — open a fresh one with markdone in URL
  ...
  // Normal tap — just open/focus the app
  e.waitUntil(
    self.clients.matchAll({type:'window', includeUncontrolled:true}).then(clients => {
      for (const client of clients) {
        if ('navigate' in client) {
