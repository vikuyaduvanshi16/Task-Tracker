// Learner Tracker — Service Worker v9
const SW_VERSION = 'ait-sw-v9';
...
self.addEventListener('notificationclick', e => {
  ...
  if (action === 'done') {
    // Auto-tick: navigate existing window OR open fresh with ?markdone=
    e.waitUntil(
      self.clients.matchAll({ type: 'window', includeUncontrolled: true })
        .then(clients => {
          ...
          if (clients.length > 0) {
            const c = clients[0];
            c.focus();
            // navigate() reloads the page with the markdone param
            if (typeof c.navigate === 'function') return c.navigate(markUrl);
            // fallback: postMessage if navigate not supported
            c.postMessage({ type: 'MARK_DONE', taskId: data.taskId });
            return;
          }
  ...
  if (action === 'skip') {
    // Do nothing — just close (already closed above)
    return;
  }

  // Regular tap on notification body — open/focus app
  e.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true })
      .then(clients => {
...
function scheduleAll(p) {
  ...
  [
    { key: 'morningTime', icon: '🌅', title: 'Good morning, Viku!',
      body: p => `You have ${p.totalTasks} tasks today. First up: ${p.firstTask}` },
    { key: 'eveningTime', icon: '🌆', title: 'Evening Summary',
      body: p => `Done today: ${p.checkedIds.length}/${p.totalTasks} tasks ✅` },
    { key: 'customTime1', icon: '⏰', title: 'Custom Reminder',
      body: p => p.customMsg1 || 'Time to check your tracker.' },
    { key: 'customTime2', icon: '⏰', title: 'Custom Reminder',
      body: p => p.customMsg2 || 'Time to check your tracker.' },
  ].forEach(s => {
    if (!p[s.key]) return;
  ...
  console.log('[SW] Total notifications scheduled:', scheduled);
}
