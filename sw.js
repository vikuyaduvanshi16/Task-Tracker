// Learner Tracker — Service Worker v7 (restored + auto-tick fix)
const SW_VERSION = 'ait-sw-v7';
  ...
  if (action === 'done') {
    e.waitUntil(
      self.clients.matchAll({type:'window', includeUncontrolled:true}).then(clients => {
        const markUrl = (data.url || '/') + '?markdone=' + data.taskId;
        if (clients.length > 0) {
          // App is open — send message to auto-tick
          clients[0].focus();
          clients[0].postMessage({type:'MARK_DONE', taskId:data.taskId});
          return;
        }
        // App is closed — open with markdone in URL
